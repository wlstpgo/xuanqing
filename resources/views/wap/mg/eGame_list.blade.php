...
@section('content')
    ...
    <ul class="clear textCenter">
        ...
        @foreach($mg_gameId as $k => $item)
            <li>
                <a href="{{ route('mg.playGame') }}?gametype={{ $item }}&devices=1" class="link-box">
                    <div class="link-box-pic">
                        <img src="{{ asset('/wap/images/mg')}}/{{ $item }}.png" alt="">
                    </div>
                    <p class="link-box-txt">{{ $mg_zh[$k] }}</p>
                </a>
                {{--<a href="{{ route('mg_mobile.playGame') }}?gameName={{ $item }}"> --}}
                    {{--<img src="{{ asset('/wap/images/mg')}}/{{ $item }}.png" style="width: 70px;height: 70px;">--}}
                    {{--<p class="collect">{{ $mg_zh[$k] }}</p>            --}}
                    {{--<a class="start_game" href="{{ route('mg_mobile.playGame') }}?gameName={{ $item }}">进入</a> --}}
                {{--</a>--}}
            </li>
        @endforeach
    </ul>
