...
class BbinController extends WebBaseController
{
    ...
    public function login(Request $request)
    {
        ...
        $api = Api::where('api_name', $platformCode)->where('on_line', 0)->first();
            ...
            $id = $request->get('gametype')?:'live';
            //检查账号是否注册
            $member_api = $member->apis()->where('api_id', $api->id)->first();
            if (!$member_api)
            {
                $res = json_decode($server->register('BBIN',$username,$password), TRUE);
        ...
        if ($this->getConfig()->is_transfer_on == 0)
        {
            ...
            $res = json_decode($server->login('BBIN',$username, $password, $id), TRUE);
    ...
    {
        return view('api.api_sf.getRecord', ['api_code' => 'BBIN']);
    }
